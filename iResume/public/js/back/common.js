$(function () {
  /* left-menu选中状态 */
  var $sideMenuLk = $('.br-sideleft-menu').find('a');
  $sideMenuLk.each(function () {
    var $this = $(this);
    if ($this.attr('href') === window.location.pathname) {
      $this.addClass('active');
      var $aParentPrev = $this.parent().parent('.br-menu-sub').prev();
      if ($aParentPrev.length !== 0 && $aParentPrev.hasClass('br-menu-link')) {
        $aParentPrev.addClass('active').trigger('click');
      }
    }
  })
});
